...
class MEC_feature_events extends MEC_base
{
    ...
    public function init()
    {
        ...
        if(isset($this->settings['event_gallery_status']) and $this->settings['event_gallery_status'])
        {
            ...
            $this->factory->action('wp_ajax_nopriv_mec_event_gallery_image_upload', array($this, 'gallery_image_upload'));
        }
        ...
        ?>
            ...
            <script type="text/javascript">
                jQuery(document).ready(function($) {
                    // Initialize datepicker with yearRange from 1990 to the current year
                    var currentYear = new Date().getFullYear();
                    $('#mec_start_date, #mec_end_date').datepicker({
                        dateFormat: 'yy-mm-dd', // Ensure the format matches your requirements
                        changeMonth: true,
                        changeYear: true,
                        yearRange: '1900:' + currentYear, // Limit to 1990 to current year
                        maxDate: new Date()
                    });
                });
            </script>
					...
					<div class="box top">
						...
						<div class="content">
                            <p><?php esc_attr_e('The value of this option will be replaced by the single event page link on shortcodes. Insert full link including http(s):// - Also, if you use an advertising URL, you can use the URL Shortener.', 'modern-events-calendar-lite'); ?>
                                <a href="https://bit.ly/" target="_blank" rel="noopener noreferrer"><?php esc_html_e('URL Shortener', 'modern-events-calendar-lite'); ?></a>
                            </p>
                        </div>
					</div>
					<i title="" class="dashicons-before dashicons-editor-help"></i>
    ...
    public function filter_columns($columns)
    {
        ...
        // Sold Tickets
        if($this->getPRO() && isset($this->settings['booking_status']) && $this->settings['booking_status']) $columns['tickets'] = esc_html__('Tickets', 'modern-events-calendar-lite');
