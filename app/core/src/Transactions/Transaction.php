...
class Transaction {
	...
	public function __construct( $transaction_id, $data = null ) {
		...
		$this->transaction_id = $transaction_id ?: static::generate_transaction_id();
	...
	public function prepare_data( $data ) {
		...
		return wp_parse_args(
			...
			array(
				...
				'coupon' => '',
				'fields' => array(),
				'gateway' => $gateway ?: 'MEC_gateway_pay_locally',
	...
	public function get_ticket_discounts_details( $ticket, $total_tickets_amount_with_variations ) {
		...
		if ( $coupon ) {
            ...
            if ( $coupon_id ) {
                ...
                $maximum_discount = $coupon_details['maximum_discount'] ?? 0;
                ...
                if($total_ticket_discount > $maximum_discount && $maximum_discount!=0){
                    $discount_ticket_amount = $maximum_discount;
                    $discount_ticket_variation_amount = $ticket_variations_amount - $maximum_discount < 0 ? 0 : $ticket_variations_amount - $maximum_discount;
                    $total_ticket_discount = $discount_ticket_amount + $discount_ticket_variation_amount;
                }
	...
	public function get_mec_coupon_detail( $coupon ) {
        ...
        $maximum_discount = get_metadata('term', $coupon_id, 'maximum_discount', true);
		if ( $coupon_id ) {
			...
			$coupon_details['maximum_discount'] = $maximum_discount;
		}
